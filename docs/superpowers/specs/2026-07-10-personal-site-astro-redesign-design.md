...
- 第一版不实现 CMS、评论、站内搜索、动态筛选、访问统计和用户可切换主题。固定的深色沉浸式视觉属于设计系统，不属于主题切换功能。
...
使用「沉浸式叙事＋温暖个人工作室＋Bento 工作台」融合方向。参考 `zhangtenggan.cn` 的单页叙事节奏、固定锚点导航、编号章节和粒子首屏，但不复制其文案、素材、粒子分布、精确配色、字体或尺寸。
...
- 暖白主文字与低饱和次级文字。
- 珊瑚色到紫色的有限渐变作为强调色。
- 编号章节、细分割线、圆角编辑型卡片和克制阴影。
- 亲近、有创造力，但不使用黑客控制台、过度科技感或儿童化装饰。
- 首页与全部子页面复用相同的导航、页头、章节标题、卡片、间距、按钮和页脚语言。

首页允许使用更强的粒子背景、打字机文案和 Bento 密度；子页面以可读性为主，通过相同 token 和组件保持一致，而不是复制首页布局。
...
- `ParticleHero`：首页粒子、打字机和 reduced-motion 降级。
- `PageIntro`／`SectionHeading`：子页面标题和编号章节一致性。
...
- 桌面端与移动端符合已确认的沉浸式叙事与温暖 Bento 融合设计。
- 首页与所有子页面共享同一套视觉 token、导航、页头、章节、卡片和页脚组件。

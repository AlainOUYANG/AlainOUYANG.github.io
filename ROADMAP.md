...
Astro 个人网站重建已进入实施阶段。当前生产站仍由 Jekyll 和 AcademicPages 构建；新站先在 `site/` 中独立实现和验证，不改变生产发布源。
...
- 编写 Astro 重建实施计划并同步参考站融合要求。

## 待办

1. 在不改变生产发布源的前提下搭建 `site/` Astro 静态站骨架。
2. 建立 Content Collections schema 与结构化个人数据。
3. 实现统一设计系统、全局布局、导航和沉浸式粒子首屏。
4. 实现首页 Bento 以及专题、文章、AI 实验室、研究、关于等子页面。
5. 迁移有效静态资源、论文、CV 和历史文章。
6. 建立旧站 URL manifest，验证迁移后的路径兼容性。
7. 实现 SEO、RSS、sitemap、robots.txt 和 404。
8. 完成类型检查、生产构建、链接检查、Playwright、截图与 Lighthouse 验证。
9. 用户确认后再调整 GitHub Pages 发布方式。
10. Astro 生产站验证稳定且用户确认后，再决定是否删除旧 Jekyll 文件。

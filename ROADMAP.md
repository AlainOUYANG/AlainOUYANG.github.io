...
- 2026-08-08：主站前端五项优化上线——`master` 推送后 Actions 运行 `29817700283` 的 build 与 deploy 成功，线上抽查确认首页「精选文章整理中」引导行、文章 meta description 与上一篇导航均生效。
- 2026-07-19：完成主站前端五项优化并本地验证——①重写 134 篇历史文章的坏摘要（原为迁移残留的 `date: YYYY-MM-DD`，现取正文首段截断，修复列表摘要与 meta/og/JSON-LD 描述）；②首页「最近写作」改为只展示 `featured` 文章，暂无精选时显示「精选文章整理中」引导行；③首页大标题用 `Intl.Segmenter` 按词分段断行，避免词内断开；④专题／文章／研究列表行收敛为整行单链接，文章详情新增上一篇／下一篇导航；⑤`--section-space` 收紧为 clamp(3.25rem, 6vw, 5rem)。`verify:all` 七项契约通过，本地 preview 目检首页、文章列表与文章详情。
...
2. 持续整理因果推断、Uplift Modeling 与智能营销的精选文章；为精选文章标记 `featured: true` 以充实首页「最近写作」。
